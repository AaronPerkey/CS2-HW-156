...
public class DataConverter {
			...
			while (s.hasNext()) {
				...
				if (!line.trim().isEmpty()) {
					...
					if (tokens[1].equals("E")) {
						...
						it = new Equipment(itemCode, name, model);
					} else if (tokens[1].equals("S")) {
						double hourlyRate = Double.parseDouble(tokens[3]);
						it = new Service(itemCode, name, hourlyRate);
					} else if (tokens[1].equals("P")) {
						String unit = tokens[3];
						double price = Double.parseDouble(tokens[4]);
						it = new Product(itemCode, name, unit, price);
					}
		...
		try (Scanner s = new Scanner(f)) {
			...
			while (s.hasNext()) {
				...
				if (!line.trim().isEmpty()) {
					...
					st = new Store(storeCode, managerCode, address);
		...
		try (Scanner s = new Scanner(f)) {
			...
			while (s.hasNext()) {
				...
				if (!line.trim().isEmpty()) {
					...
					in = new Invoice(invoiceCode, storeCode, customer, salesPerson, date);
	...
	public static void main(String args[]) {
		...
		try {
			JsonLoader.writeJSONPersons(parseDataFilePerson(), "data/Persons.json");
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
